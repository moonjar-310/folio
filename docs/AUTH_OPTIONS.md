...
Both runtimes issue a Folio HS256 Access JWT valid for **300 seconds** and a random Refresh Token valid for **604800 seconds (one week)**. The browser keeps the Access JWT and CSRF value in memory and sends the JWT as `Authorization: Bearer`. A one-shot timer refreshes when ten seconds remain; API calls and tab focus/page restoration/visibility changes also check that deadline. A shared browser-instance mutex serializes refresh requests. An unexpected API 401 triggers one refresh and one retry, never a retry loop. Reloading restores authentication through `POST /api/auth/refresh`; tokens are never written to localStorage or sessionStorage.

A rejected refresh or a second API 401 immediately removes the workspace and shows sign-in. Cloudflare sessions automatically reload the protected page to enter Access sign-in; API redirects are intercepted before following them to avoid disguising Access redirects as CORS errors. Reload occurs only after verifying browser backups for unsaved editor and Quick Note text. If backup fails, the sign-in screen remains in the same tab with drafts still in memory and an error explaining the failure. Network failures and server errors alone do not trigger logout or redirect. A failed background refresh can be retried on the next API call or tab return.
