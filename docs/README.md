...
- [Authentication and token lifecycle](AUTH_OPTIONS.md)
- [Folder behavior and recovery](FOLDERS.md)
...
The browser is Leptos CSR. The shared Rust application selects native filesystem/SQLite or Cloudflare R2/D1 through an adapter crate. Markdown paths are canonical. File listings enumerate storage keys and use cached SQL metadata; search/planner queries use SQL indexes. SQL also owns authentication, refresh sessions and mutation leases, so the whole database is not disposable.
