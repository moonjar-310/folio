...
if(!/^http:\/\/(127\.0\.0\.1|localhost):879[01]$/.test(base??''))throw new Error('Use the disposable path-storage test servers on 8790/8791.');
let cookie='',csrf='';
async function call(method,path,body={}) {
 const r=await fetch(base+path,{method,signal:AbortSignal.timeout(30000),headers:{'content-type':'application/json',cookie,'x-csrf-token':csrf},body:method==='GET'?undefined:JSON.stringify(body)});
...
csrf=(await call('POST','/api/auth/login',{username:'folio-test',password:'Folio-local-test-2026!'})).csrf;
if(phase==='prepare') {
...
} else throw new Error('Expected prepare or verify-loss');
